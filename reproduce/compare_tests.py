...
# indep_var_name = "Sparsity inside tested group ($k_1$)"
...
# indep_var_name = "Sparsity outside tested group ($k_2$)"

# Figure 2D, 3A, 3B, 15D, 15E, 16D
# num_points = 6
# n = np.full(num_points, 100)
# d = np.full(num_points, 50)
# k = np.full(num_points, 10)
...
# indep_var_name = r"Correlation ($\rho$)"
...
def run_tests(task_id, alpha=0.05):
    """
    ...
    for i in range(its_per_job):
        for j in range(num_points):
            ...
            r_seed = 1000 * task_id + num_points * i + j
            bonf_pval = bonf_ell(y, X, k[j], r_seed)
...
if __name__ == "__main__":
    ...
    with run_tests(task_id, alpha=***).
    """
    test_rejections = run_tests(task_id)

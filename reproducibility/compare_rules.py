...
def generate_penalties(y, X, V):
    ...
    model.fit(X, y_tilde)                                            
    # min rule
    lambda_min = model.alpha_
    # 1se rule
    alphas = model.alphas_
    mean_cv_errors = np.mean(model.mse_path_, axis=1)
    std_cv_errors = np.std(model.mse_path_, axis=1)
    ...
    lambda_1se = alphas[alpha_1se_index]
...
def power(sig, spars_1, spars_2, rho):
    ...
    for i in range(1):
        ...
        l_min, l_1se = generate_penalties(y, X, V)
        # Calc p-values
        p_L_1 = L_pval(y, X, V, l_min)
        p_L_2 = L_pval(y, X, V, l_1se)
        # Reject/retain
        if (p_L_1 <= alpha):
            rejects_L_1 += 1
        if (p_L_2 <= alpha):
            rejects_L_2 += 1
    return rejects_L_1, rejects_L_2
